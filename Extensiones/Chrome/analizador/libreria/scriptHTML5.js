 ...
 * @param  {string} html - Contains the browser's HTML code. / Contiene el código HTML del navegador.
 * @return {array} resultAnalisis - Returns the warnings corresponding to the analysis. / Retorna las advertencias correspondientes al análisis.
 */
...
function traducirLinea(linea, htmlOriginal, html, getAdvertencia){
	...
	var k = 0;	// iterador
	var w = 0;  // iterador
	var lineaCopy = []; //Vector backug / Vector de copia
	var extracto = []; // Abstract warnings / Resumen de la advertencia
	...
	var vectLineaAdv = []; //save  the value of warnings produced in the syntactic and semantic analysis / guarda el valor de las advertencias producidas en el analisis sintáctico y semántico
	...
	for(var i in getAdvertencia){
		...
		vectLineaAdv.push(parseInt(lin[3]));

	}
	// ordered warnings Ascending / ordena las advertencias de menor a mayor 
	var aux, aux2;
         for(var i=0;i<vectLineaAdv.length-1;i++){
              for(var j=0;j<vectLineaAdv.length-i-1;j++){
                   if(vectLineaAdv[j+1]<vectLineaAdv[j]){
                      aux=vectLineaAdv[j+1];
                      aux2=getAdvertencia[j+1];
                      vectLineaAdv[j+1]=vectLineaAdv[j];
		...
		for(var i= 0; i<vectLineaAdv.length; i++){
			var posInicio = lineaCopy[paralelo[vectLineaAdv[i]]-1].indexOf(htmlOriginal[vectLineaAdv[i]]);
			var posFinal = htmlOriginal[vectLineaAdv[i]].length;
			var ext;
			if(lineaCopy[paralelo[vectLineaAdv[i]]-1].indexOf(htmlOriginal[vectLineaAdv[i]]) !== -1)
				ext = lineaCopy[paralelo[vectLineaAdv[i]]-1].substring(posInicio,posInicio+posFinal)
			else
				ext = lineaCopy[paralelo[vectLineaAdv[i]]-1];
			getAdvertencia[i] = {
				getAdvertencia:getAdvertencia[i].replace((new RegExp("("+vectLineaAdv[i]+")")),paralelo[vectLineaAdv[i]]),
				extracto:ext
			};					
		}			         
